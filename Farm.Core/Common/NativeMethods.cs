...
namespace Farm.Core.Common
{
    internal static class NativePowerMethods
    {
        public static void Reboot()
        {
            ...
            try
            {
                // get processController token
                if (!OpenProcessToken(Process.GetCurrentProcess().Handle,
                    TOKEN_QUERY | TOKEN_ADJUST_PRIVILEGES,
                    out tokenHandle))
                {
                    throw new Win32Exception(Marshal.GetLastWin32Error(),
                        "Failed to open processController token handle");
                }
                ...
                // add the shutdown privilege to the processController token
                ...
                {
                    throw new Win32Exception(Marshal.GetLastWin32Error(),
                        "Failed to adjust processController token privileges");
                }

                // reboot
                if (!ExitWindowsEx(ExitWindows.Reboot | ExitWindows.Force | ExitWindows.ForceIfHung,
            ...
            {
                // close the processController token
