...
namespace ProcessController
{
    ...
    public class ProcessController : ICommunicationBase, IProcessController
    {
        ...
        public PrcessStatus Execute(CheckProcess checkProcess)
        {
            return CheckPrcessStatus(checkProcess);
        }

        public PrcessStatus Execute(RestartProcess rebootProcess)
        {
                    ...
                    do
                    {
                        do
                        {
                            if (mylocInfo.HasExited)
                            {
                                ...
                                bus.Publish(new PrcessStatus { Name = proLocInf.Name ,StatusProcess = PrcessStatus.ProcStatus.NotRunning, Status = MessageStatus.Info});
                                break;
                            }
                            ...
                            if (!mylocInfo.Responding)
                            {
                                try
                                {
                                    ...
                                    bus.Publish(new PrcessStatus { Name = proLocInf.Name, StatusProcess = PrcessStatus.ProcStatus.NotResponding , Status = MessageStatus.Info });
                        ...
                        bus.Publish(new PrcessStatus { Name = proLocInf.Name, StatusProcess = PrcessStatus.ProcStatus.Running, Status = MessageStatus.Info });
                    } while (!_token.IsCancellationRequested);
                }, _token.Token);
            }
        }
